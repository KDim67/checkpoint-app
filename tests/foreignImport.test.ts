...
describe('parseTrelloBoard', () => {
  ...
  it('places each card in its own list', () => {
    const board = parseTrelloBoard(trello)!
  ...
  it('carries the description and the due date', () => {
    const board = parseTrelloBoard(trello)!
    expect(board.cards.find(c => c.title === 'Player movement')!.body).toBe('WASD')
    expect(board.cards.find(c => c.title === 'Jump feel')!.due_at).toBe(Date.parse('2026-06-15T09:00:00.000Z'))
  })
  ...
  it('does not invent a priority Trello never had', () => {
    for (const card of parseTrelloBoard(trello)!.cards) {
      expect(card.priority).toBe(0)
    }
  })

  it('brings checklists across in their own order', () => {
    const card = parseTrelloBoard(trello)!.cards.find(c => c.title === 'Player movement')!
  ...
  it('skips cards with no title', () => {
    const board = parseTrelloBoard({ ...trello, cards: [{ id: 'e', name: '   ', idList: 'l1', pos: 1 }] })!
...
describe('collectLabels', () => {
  it('gathers each distinct label once, with its colour', () => {
    expect(collectLabels(parseTrelloBoard(trello)!)).toEqual([
...
describe('describeImport', () => {
  it('counts what is about to be created', () => {
    expect(describeImport(parseTrelloBoard(trello)!)).toBe('3 columns · 3 cards')
  })
})
